    ...
    <xsl:template name="opt">
        <xsl:variable name="argLabel">
            <xsl:choose>
                ...
                <xsl:when test="set_arg_label">{ \it <xsl:value-of select="set_arg_label"/>}
                </xsl:when>
        ...
        <xsl:for-each select="alias">
            {\tt -<xsl:value-of select="."/>}<xsl:value-of select="$argLabel"/>
...
<xsl:text>
        ...
        <xsl:if test="value">
            ...
            <xsl:for-each select="value">
                <xsl:for-each select="alias">{\tt <xsl:value-of select="."/>}
                    <xsl:if test="following-sibling::alias">,</xsl:if>
                    <xsl:text> </xsl:text>
                </xsl:for-each>
                &amp;
                <xsl:apply-templates mode="to_latex" select="long_desc"/>\\
    ...
    <xsl:template match="use_arg_label" mode="to_latex">
        <xsl:choose>
            <xsl:when test="ancestor::*/set_arg_label">{\it <xsl:value-of select="ancestor::*/set_arg_label"/>}
            </xsl:when>
            <xsl:otherwise>{\it arg}</xsl:otherwise>
        </xsl:choose>
    </xsl:template>
