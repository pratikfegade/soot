...
public abstract class Case extends Stmt implements Cloneable {
    ...
    private boolean isDAbefore_compute(Variable v) {
        return getParent().getParent() instanceof Block && ((Block) getParent().getParent()).isDAbefore(v)
                && super.isDAbefore(v);
    }
    ...
    public boolean isDUbefore(Variable v) {
        ASTNode$State state = state();
        try {
            return getParent().getParent() instanceof Block && ((Block) getParent().getParent()).isDUbefore(v)
                    && super.isDUbefore(v);
        } finally {
        }
    }
