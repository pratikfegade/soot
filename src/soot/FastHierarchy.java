...
{    
    ...
    private MultiMap<SootClass,SootClass> interfaceToImplementers =
            new ConcurrentHashMultiMap<>();

    /** This map is a transitive closure of interfaceToSubinterfaces,
     * and each set contains its superinterface itself. */
    private MultiMap<SootClass,SootClass> interfaceToAllSubinterfaces =
            new ConcurrentHashMultiMap<>();
    ...
    private MultiMap<SootClass,SootClass> interfaceToAllImplementers =
            new ConcurrentHashMultiMap<>();
    ...
    private Map<SootClass, Interval> classToInterval = new HashMap<>();
    ...
    protected class Interval {
        ...
        boolean isSubrange( Interval potentialSubrange ) {
            return potentialSubrange != null && lower <= potentialSubrange.lower && upper >= potentialSubrange.upper;
        }
    }
    private int dfsVisit(int start, SootClass c) {
    ...
    private Set<SootClass> getAllImplementersOfInterface(SootClass parent) {
     ...
     * */
    private Set<SootClass> getAllSubinterfaces(SootClass parent) {
        parent.checkLevel(SootClass.HIERARCHY);
    ...
    public boolean canStoreType( Type child, Type parent ) {
        ...
        if( child instanceof RefType ) {
            return parent.equals(Scene.getInstance().getObjectType()) || parent instanceof RefType && canStoreClass(((RefType) child).getSootClass(), ((RefType) parent).getSootClass());
            ...
            if (achild.numDimensions == aparent.numDimensions) {
                return achild.baseType.equals(aparent.baseType) || achild.baseType instanceof RefType && aparent.baseType instanceof RefType && canStoreType(achild.baseType, aparent.baseType);
            } else
                return achild.numDimensions > aparent.numDimensions && (aparent.baseType.equals(RefType.getInstance("java.lang.Object")) || aparent.baseType.equals(RefType.getInstance("java.io.Serializable")) || aparent.baseType.equals(RefType.getInstance("java.lang.Cloneable")));
        } else
        	return false;
    }
     ...
     * even though some objects implementing the child interface may also
     * implement the parent interface. */
    private boolean canStoreClass(SootClass child, SootClass parent) {
        parent.checkLevel(SootClass.HIERARCHY);
        ...
        if( childInterval == null ) { // child is interface
            ...
            for (SootClass anImpl : impl) {
                parentInterval = classToInterval.get(anImpl);
   ...
   */
    private Collection<SootClass> getSubclassesOf(SootClass c) {
