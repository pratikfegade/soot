 ...
 * exceptional flow as well as START/STOP auxiliary nodes. In a nutshell, the
 * exceptional flow is represented at the level of try-catch-finally blocks
...
public class EnhancedUnitGraph extends UnitGraph {
	...
	protected void handleExplicitThrowEdges() {
		MHGDominatorTree<Unit> dom = new MHGDominatorTree<Unit>(
				new MHGDominatorsFinder<Unit>(this));
		MHGDominatorTree<Unit> pdom = new MHGDominatorTree<Unit>(
				new MHGPostDominatorsFinder<Unit>(this));
		...
		TailsLoop: for (Iterator<Unit> itr = tails.iterator(); itr.hasNext();) {
			...
			if (x2mergePoint.containsKey(xgode))
				...
				for (Iterator<DominatorNode<Unit>> domItr = domChilds
						.iterator(); domItr.hasNext();) {
					...
					List<Unit> path = this.getExtendedBasicBlockPathBetween(
							childGode, tail);
	...
	protected void addAuxiliaryExceptionalEdges() {
		...
		AddExceptionalEdge: for (Iterator<Trap> trapIt = body.getTraps()
				.iterator(); trapIt.hasNext();) {
			...
			/**
			 * Check if this trap is a finally trap that handles exceptions of
			 * an adjacent catch block; what differentiates such trap is that
			 * it's guarded region has the same parent as the handler of the
			 * trap itself, in the dom tree.
			 * 
			 * The problem is that we don't have a complete DOM tree at this
			 * transient state.
			 * 
			 * The work-around is to not process a trap that has already an edge
			 * pointing to it.
			 * 
			 */
			...
			if (this.unitToPreds.containsKey(handler)) {
				...
				for (Iterator<Unit> preditr = handlerPreds.iterator(); preditr
						.hasNext();)
...
/**
 * This class represents a block of code guarded by a trap. Currently, this is
 * not used but it might well be put to use in later updates.
 * 
 * @author Hossein Sadat-Mohtasham
 *
 */
 ...
 *         beginning of a try block at the Jimple level. This is going to be
 ...
 * 
 *         This class represents a special nop statement that marks the
 *         beginning of method body at the Jimple level. This is going to be
 *         used in the CFG enhancement.
 *
 */
@SuppressWarnings("serial")
 ...
 * @author Hossein Sadat-Mohtasham Feb 2010
 * 
 *         This class represents a special nop statement that marks the exit of
 *         method body at the Jimple level. This is going to be used in the CFG
 *         enhancement.
 *
 */
