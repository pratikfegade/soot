	...
	public void preprocess(DavaBody body, AugmentedStmtGraph asg) {
		...
		// Find the first approximation for all the try catch bodies.
		{
			for (Trap trap : body.getTraps()) {
				...
				enlist.add(new ExceptionNode(tryBody, trap.getException(), asg
						.get_AugStmt((Stmt) trap.getHandlerUnit())));
			}
		}
			...
			for (ExceptionNode en : enlist) {
				...
				for (AugmentedStmt tras : try_body) {
					for (AugmentedStmt pas : tras.cpreds) {
						...
						if ((try_body.contains(pas) == false)
								&& (ps instanceof GotoStmt)) {
							boolean add_it = true;
				...
				for (int i = 0; i < ena.length - 1; i++) {
					...
					for (int j = i + 1; j < ena.length; j++) {
						...
						if ((eniTryBody.equals(enjTryBody) == false)
							...
							IterableSet<AugmentedStmt> newTryBody = eniTryBody
									.intersection(enjTryBody);

							if (newTryBody.equals(enjTryBody))
								eni.splitOff_ExceptionNode(newTryBody, asg,
										enlist);
							else
								enj.splitOff_ExceptionNode(newTryBody, asg,
										enlist);
				...
				for (ExceptionNode en : enlist) {
					...
					while (worklist.isEmpty() == false) {
						...
						for (AugmentedStmt sas : as.csuccs) {
							if ((tryBody.contains(sas) == false)
									|| (touchSet.contains(sas)))
								continue;
			...
			for (ExceptionNode en : enlist) {
				...
				for (IterableSet<AugmentedStmt> bucketTryBody : bucket) {
					if (bucketTryBody.equals(curTryBody)) {
						repExceptionNode = tryBody2exceptionNode
								.get(bucketTryBody);
						break;
					}
				}
