...
public class SynchronizedBlockFinder implements FactFinder {
	...
	public void find(DavaBody body, AugmentedStmtGraph asg, SETNode SET)
				...
				if (synchSet != null) {
					...
					if (previousStmt != null) {
						...
						if (previousS instanceof DefinitionStmt) {
							...
							if (rightPrevious.toString().compareTo(
									local.toString()) == 0) {
					...
					while (enit.hasNext()) {
						...
						if (verify_CatchBody(en, synchBody, local, copiedLocal)) {
							if (SET.nest(new SETSynchronizedBlockNode(en, local))) {
								...
								while (ssit.hasNext()) {
									AugmentedStmt ssas = (AugmentedStmt) ssit
											.next();
		...
		for (AugmentedStmt as : asg) {
			if ((as.get_Stmt() instanceof MonitorStmt)
					&& (usedMonitors.contains(as) == false)) {
				monitorFacts.add(as);
			}
		}
	}
	...
	private void find_VariableIncreasing(AugmentedStmtGraph asg,
			HashMap local2level_template, LinkedList<AugmentedStmt> viAugStmts,
		...
		while (worklist.isEmpty() == false) {
			...
			while (sit.hasNext()) {
				...
				HashMap<Value, Integer> slocal2level = (HashMap<Value, Integer>) as2rml
						.get(sas);
	 ...
	 */
	private void DFS_Scc(AugmentedStmt as, IterableSet component,
			HashMap as2rml, HashMap as2color, AugmentedStmt seedStmt,
			IterableSet viSeeds) {
		as2color.put(as, GRAY);

		Stmt s = as.get_Stmt();
		// get local to level mapping of the augmented stmt
		HashMap<Value, Integer> local2level = (HashMap<Value, Integer>) as2rml
				.get(as);
		...
		while (sit.hasNext()) {
			...
			if (scolor.equals(WHITE)) {
				...
				while (mlsit.hasNext()) {
					...
					if (slocal2level.get(local).intValue() < local2level.get(
							local).intValue()) {
		...
		monitorEnterLoop: while (mesit.hasNext()) {
			...
			while (worklist.isEmpty() == false) {
				...
				if ((s instanceof DefinitionStmt)
						&& (((DefinitionStmt) s).getLeftOp() == local))
	...
	private void set_MonitorLevels(AugmentedStmtGraph asg) {
		...
		while (asgit.hasNext()) {
			...
			as2ml.put(asgit.next(),
					(Map<Value, Integer>) local2level_template.clone());
		}
		...
		find_VariableIncreasing(asg, local2level_template, viAugStmts,
				incrAs2locals);
		/*
	 ...
	 */
	private void removeOtherDominatedStmts(IterableSet synchBody,
			AugmentedStmt sas) {
	...
	private boolean verify_CatchBody(ExceptionNode en, IterableSet synchBody,
			Value monitorVariable, Value copiedLocal) {
				...
				if (as.get_Stmt() instanceof ExitMonitorStmt) {
					...
					while (succIt.hasNext()) {
						...
						if (as1.get_Stmt() instanceof GotoStmt) {
							Unit target = ((soot.jimple.internal.JGotoStmt) as1
									.get_Stmt()).getTarget();
							if (target instanceof DefinitionStmt) {
								...
								if (!(asnFrom instanceof CaughtExceptionRef)) {
									...
									Iterator localIt = davaBody.getLocals()
											.iterator();
									String typeName = "";
									while (localIt.hasNext()) {
										Local local = (Local) localIt.next();
										if (local.toString().compareTo(
												leftOp.toString()) == 0) {
									...
									if (!(typeName.compareTo(THROWABLE) == 0)) {
										...
										// synchBody which are dominated by this
										// stmt
										removeOtherDominatedStmts(synchBody,
												as1);
									} else {
		...
		if (synchEnter == null) {
			throw new RuntimeException(
					"Could not find enter stmt of the synchBody: "
							+ davaBody.getMethod().getSignature());
		}
		...
		if (entryPoint.get_Stmt() instanceof GotoStmt) {
			...
			entryPointTarget = ((soot.jimple.internal.JGotoStmt) entryPoint
					.get_Stmt()).getTarget();
		}
		...
		while (as.get_Stmt() instanceof GotoStmt) {
			...
			if (entryPointTarget != null) {
				if ((as.get_Stmt() != entryPointTarget)
						&& (as.cpreds.size() != 1)) {
					if (as.cpreds.size() != 1) {
		...
		while (s instanceof DefinitionStmt
				&& (((DefinitionStmt) s).getRightOp().toString()
						.compareTo(throwlocal.toString()) == 0)) {
	 ...
	 */
	private IterableSet get_BodyApproximation(AugmentedStmt head,
