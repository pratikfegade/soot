...
final class AsmMethodSource implements MethodSource {
    ...
    AsmMethodSource(String name, int maxLocals, InsnList insns, List<LocalVariableNode> localVars, List<TryCatchBlockNode> tryCatchBlocks) {
        ...
        if (this.name.equals("<org.clyze.jphantom.Driver.run>")) {
            for (LocalVariableNode lvn : localVars) {
                System.out.println(lvn.name + " slot: " + lvn.index +
                        " start: " + labelToLineNodeMap.get(lvn.start.getLabel()) +
                        " end: " + labelToLineNodeMap.get(lvn.end.getLabel()));
            }
        }
    }
    ...
    private Local getLocal(int idx) {
        ...
        Integer i = idx;
        Local l = locals.get(i);  // First search the list of stack locals for a variable with index idx
        if (l == null) {
            String name = null;
            String desc = null;
            int startScope = -1;
            ...
            if (localVars != null) {       // If not found in stack locals search method local variables
                for (LocalVariableNode lvn : localVars) {
                    if (lvn.index == idx) {
                        ...
                        if (this.name.contains("<org.clyze.jphantom.Driver.run>")) {
            ...
            if (desc == null) {
                l = Jimple.v().newLocal(name, UnknownType.v(), startScope, endScope);
            }
            else {
                l = Jimple.v().newLocal(name, jimpleTypeOfDescriptor(desc), startScope, endScope);
            }
            locals.put(i, l);
        }
        else {
            if (name.contains("<org.clyze.jphantom.Driver.run>"))
                System.out.println("index found in stack");
        }
//        else {
//            for (LocalVariableNode lvn : localVars) {
//                if (lvn.index == idx) {
...
//        }
    ...
    Local newStackLocal() {
        ...
        Local l = Jimple.v().newLocal("$stack" + idx, UnknownType.v(), -1, -1);
        locals.put(idx, l);
        return l;
    }
