...
public class AccessManager {
	...
	/**
	 * Returns true if an access to <code>target</code> is legal from code in
	 * <code>container</code>.
	 */
	...
	/**
	 * Returns true if the statement <code>stmt</code> contains an illegal
	 ...
	 * accessor methods. <code>before</code> and <code>after</code> can be null
	 * to indicate beginning/end respectively.
	 * 
	 * @param body
	 * @param before
	 * @param after
	...
	private static void createSetAccessor(SootMethod container, AssignStmt as, FieldRef ref) {
		...
		if (accessor == null) {
			...
			if (ref instanceof InstanceFieldRef) {
				accStmts.add(
						Jimple.v().newIdentityStmt(thisLocal, Jimple.v().newParameterRef(target.getType(), paramID)));
				parameterTypes.add(target.getType());
				paramID++;
			}
