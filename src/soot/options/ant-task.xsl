...
>
    ...
    <xsl:template mode="paths" match="listopt">
        if(
        <xsl:value-of select="translate(alias[last()],'-. ','___')"/> != null ) addPath("-<xsl:value-of
            select="alias[last()]"/>", <xsl:value-of select="translate(alias[last()],'-. ','___')"/>);
    </xsl:template>
    ...
    <xsl:template mode="parse" match="multiopt">
        ...
        throw new BuildException("Bad value "+arg+" for option <xsl:value-of
    ...
    <xsl:template mode="parse" match="listopt">
        ...
        <xsl:value-of select="translate(alias[last()],'-. ','___')"/> = appendToPath(<xsl:value-of
            select="translate(alias[last()],'-. ','___')"/>, arg);
        }
    ...
    <xsl:template mode="vars" match="listopt">
        ...
        public void set_<xsl:value-of select="translate(alias[last()],'-. ','___')"/>( List setting ) {
        <xsl:value-of select="translate(alias[last()],'-. ','___')"/> = setting; }
        private List
    ...
    <xsl:template mode="phaselist" match="phaseopt">
        <xsl:for-each select="phase|radio_phase|phase/sub_phase|radio_phase/sub_phase">
            ...
            <xsl:for-each
                    select="multiopt|intopt|flopt|stropt|section/multiopt|section/intopt|section/flopt|section/stropt">
                <xsl:text/>
                public void set<xsl:value-of select="translate(alias[last()],'-. ','___')"/>(String arg) {
                addArg("-p");
                addArg("<xsl:value-of select="$phasename"/>");
