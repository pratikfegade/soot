...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:redirect="http://xml.apache.org/xalan/redirect"
    ...
    <xsl:template mode="paths" match="listopt">
        if(
        <xsl:value-of select="translate(alias[last()],'-. ','___')"/> != null ) addPath("-<xsl:value-of select="alias[last()]"/>",
        <xsl:value-of select="translate(alias[last()],'-. ','___')"/>);
    </xsl:template>
    ...
    <xsl:template mode="phaselist" match="phaseopt">
        <xsl:for-each select="phase|radio_phase|phase/sub_phase|radio_phase/sub_phase">
            ...
            <xsl:for-each select="multiopt|intopt|flopt|stropt|section/multiopt|section/intopt|section/flopt|section/stropt">
                <xsl:text/>
                public void set<xsl:value-of select="translate(alias[last()],'-. ','___')"/>(String arg) {
                addArg("-p");
                addArg("<xsl:value-of select="$phasename"/>");
