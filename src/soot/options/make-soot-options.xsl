...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:redirect="http://xml.apache.org/xalan/redirect"
    ...
    <xsl:template mode="constants" match="multiopt">
        ...
        <xsl:for-each select="value">
            ...
            <xsl:for-each select="alias">
                public static final int<xsl:copy-of select="$name"/>_<xsl:value-of select="translate(.,'-. ','___')"/> = <xsl:value-of
                    select="$number"/>;<xsl:text/>
            </xsl:for-each>
        </xsl:for-each>
    </xsl:template>
                    ...
                    */
                    ...
                    <xsl:for-each select="multiopt|section/multiopt">
                        ...
                        <xsl:for-each select="value">
                            ...
                            <xsl:for-each select="alias">
                                public static final int <xsl:value-of select="$name"/>_<xsl:value-of select="translate(.,'-. ','___')"/> =
