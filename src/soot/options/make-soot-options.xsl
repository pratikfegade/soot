...
>
    ...
    <xsl:template mode="parse" match="multiopt">
        ...
        <xsl:for-each select="value">
            ...
            &#38;&#38;
            <xsl:copy-of select="$name"/>
            !=<xsl:copy-of select="$name"/>_<xsl:value-of select="translate(alias[last()],'-. ','___')"/> ) {
    ...
    <xsl:template mode="vars" match="boolopt">
        public boolean <xsl:value-of select="translate(alias[last()],'-. ','___')"/>() { return <xsl:value-of
            select="translate(alias[last()],'-. ','___')"/>; }
        private boolean
        <xsl:value-of select="translate(alias[last()],'-. ','___')"/> = false;<xsl:text/>
        public void set_<xsl:value-of select="translate(alias[last()],'-. ','___')"/>( boolean setting ) {
        <xsl:value-of select="translate(alias[last()],'-. ','___')"/> = setting; }
    </xsl:template>
    ...
    <xsl:template mode="vars" match="multiopt">
        ...
        <xsl:for-each select="value">
            <xsl:if test="default">
                <xsl:text/>
                if(
                <xsl:value-of select="$name"/> == 0 ) return <xsl:value-of select="$name"/>_<xsl:value-of
                    select="translate(alias[last()],'-. ','___')"/>;<xsl:text/>
            </xsl:if>
        </xsl:for-each>
        return <xsl:value-of select="translate(alias[last()],'-. ','___')"/>;
        }
        public void set_<xsl:value-of select="translate(alias[last()],'-. ','___')"/>( int setting ) {
        <xsl:value-of select="translate(alias[last()],'-. ','___')"/> = setting; }
        private int
        <xsl:value-of select="translate(alias[last()],'-. ','___')"/> = 0;<xsl:text/>
    </xsl:template>
    ...
    <xsl:template mode="vars" match="listopt">
        ...
        if(
        <xsl:value-of select="translate(alias[last()],'-. ','___')"/> == null )
        return java.util.Collections.emptyList();
        else
        return <xsl:value-of select="translate(alias[last()],'-. ','___')"/>;
        }
        public void set_<xsl:value-of select="translate(alias[last()],'-. ','___')"/>( List&lt;String&gt; setting ) {
        <xsl:value-of select="translate(alias[last()],'-. ','___')"/> = setting; }
        private List&lt;String&gt;
        <xsl:value-of select="translate(alias[last()],'-. ','___')"/> = null;<xsl:text/>
    </xsl:template>
    ...
    <xsl:template mode="vars" match="stropt">
        public String <xsl:value-of select="translate(alias[last()],'-. ','___')"/>() { return <xsl:value-of
            select="translate(alias[last()],'-. ','___')"/>; }
        public void set_<xsl:value-of select="translate(alias[last()],'-. ','___')"/>( String setting ) {
        <xsl:value-of select="translate(alias[last()],'-. ','___')"/> = setting; }
        private String
        <xsl:value-of select="translate(alias[last()],'-. ','___')"/> = "";<xsl:text/>
    </xsl:template>
    ...
    <xsl:template mode="constants" match="multiopt">
        ...
        <xsl:for-each select="value">
            ...
            <xsl:for-each select="alias">
                public static final int<xsl:copy-of select="$name"/>_<xsl:value-of select="translate(.,'-. ','___')"/> =
    ...
    <xsl:template mode="usage" match="boolopt">
        +padOpt("
        <xsl:for-each select="alias">-<xsl:value-of select="."/>
        </xsl:for-each>
        ", "<xsl:apply-templates select="short_desc"/>" )<xsl:text/>
    </xsl:template>

    <!--* MULTI_OPTION *******************************************************-->
    <xsl:template mode="usage" match="multiopt">
        +padOpt("
        <xsl:for-each select="alias">-<xsl:value-of select="."/><xsl:text> </xsl:text>
        ...
        <xsl:for-each select="value">
            ...
            <xsl:if test="default">(default)</xsl:if>", "<xsl:value-of select="translate(short_desc,'&#10;',' ')"/>" )<xsl:text/>
        </xsl:for-each>
    </xsl:template>
                    ...
                    */
                    ...
                    <xsl:for-each select="boolopt|section/boolopt">
                        ...
                        <xsl:call-template name="wrap-comment">
                            <xsl:with-param name="text" select="short_desc"/>
                        </xsl:call-template>
                        .
                        <xsl:call-template name="wrap-comment">
                            <xsl:with-param name="text" select="long_desc"/>
                        </xsl:call-template>
                        */
                    ...
                    <xsl:for-each select="multiopt|section/multiopt">
                        ...
                        <xsl:for-each select="value">
                            ...
                            <xsl:for-each select="alias">
                                public static final int <xsl:value-of select="$name"/>_<xsl:value-of
                                    select="translate(.,'-. ','___')"/> = <xsl:value-of select="$number"/>;<xsl:text/>
                            </xsl:for-each>
                        </xsl:for-each>
                        /**
                        <xsl:value-of select="name"/> --
                        ...
                        */
                        ...
                        <xsl:for-each select="value">
                            ...
                            return <xsl:value-of select="$name"/>_<xsl:value-of
                                select="translate(alias[last()],'-. ','___')"/>;
                        </xsl:for-each>
    ...
    <xsl:template mode="phaselist" match="phaseopt">
        ...
        <xsl:for-each select="phase|radio_phase|phase/sub_phase|radio_phase/sub_phase">
            ...
            <xsl:for-each
                    select="boolopt|multiopt|intopt|flopt|stropt|section/boolopt|section/multiopt|section/intopt|section/flopt|section/stropt">
                <xsl:text/>
                +padOpt( "<xsl:value-of select="alias"/>
                <xsl:if test="default">(<xsl:value-of select="default"/>)
                </xsl:if>
                ", "<xsl:value-of select="translate(short_desc,'&#10;',' ')"/>" )<xsl:text/>
                <xsl:if test="value">
                    <xsl:for-each select="value">
                        +padVal( "<xsl:value-of select="alias"/>
                        <xsl:if test="default">(default)</xsl:if>", "<xsl:value-of
                            select="translate(short_desc,'&#10;',' ')"/>" )
                    </xsl:for-each>
                </xsl:if>
            </xsl:for-each>
            ;
