...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
    ...
    <xsl:template match="/options">
        ...
        <xsl:for-each select="section/phaseopt/radio_phase">
            ...
            <xsl:for-each select="sub_phase">
                ...
                <xsl:for-each select="boolopt">
                    ...
                    buttonList.add(get<xsl:value-of select="$phase_alias"/><xsl:value-of
                        select="$sub_phase_alias"/><xsl:value-of select="translate(alias[last()],'-. ','___')"/>_widget());
                    get<xsl:value-of select="$phase_alias"/><xsl:value-of select="$sub_phase_alias"/><xsl:value-of
                        select="translate(alias[last()],'-. ','___')"/>_widget().getButton().addSelectionListener(this);
                    }
        ...
        <xsl:for-each select="section/phaseopt">
            <xsl:for-each select="(phase|radio_phase)">
                ...
                <xsl:for-each select="(boolopt|stringopt|intopt|flopt|multiopt)">
                    addToEnableGroup("<xsl:value-of select="$phase_alias"/>", get<xsl:value-of
                        select="$phase_alias"/><xsl:value-of select="translate(alias[last()],'-. ','___')"/>_widget(), "<xsl:value-of
                        select="alias"/>");

                </xsl:for-each>

                <xsl:for-each select="boolopt">
                    get<xsl:value-of select="$phase_alias"/><xsl:value-of
                        select="translate(alias[last()],'-. ','___')"/>_widget().getButton().addSelectionListener(this);
                </xsl:for-each>
                ...
                <xsl:for-each select="sub_phase">
                    ...
                    <xsl:for-each select="(boolopt|stringopt|intopt|flopt|multiopt)">
                        addToEnableGroup("<xsl:value-of select="$phase_alias"/>", "<xsl:value-of
                            select="$sub_phase_alias"/>", get<xsl:value-of select="$phase_alias"/><xsl:value-of
                            select="$sub_phase_alias_translated"/><xsl:value-of
                            select="translate(alias[last()],'-. ','___')"/>_widget(), "<xsl:value-of select="alias"/>");
                    </xsl:for-each>
                    ...
                    <xsl:for-each select="section">
                        <xsl:for-each select="(boolopt|stringopt|intopt|flopt|multiopt)">
                            addToEnableGroup("<xsl:value-of select="$phase_alias"/>", "<xsl:value-of
                                select="$sub_phase_alias"/>", get<xsl:value-of select="$phase_alias"/><xsl:value-of
                                select="$sub_phase_alias_translated"/><xsl:value-of
                                select="translate(alias[last()],'-. ','___')"/>_widget(), "<xsl:value-of
                                select="alias"/>");
        ...
        */
        ...
        MessageDialog.openError(defaultShell, "Unable to find Soot Main Project", "Project "+projectName+" does not
        exist," +
        ...
        */
        ...
        <xsl:for-each select="section">
            SootOption <xsl:value-of select="translate(name[last()],'-. ','___')"/>_branch = new
            SootOption("<xsl:value-of select="name"/>", "<xsl:value-of select="translate(name[last()],'-. ','___')"/>");
            ...
            <xsl:for-each select="phaseopt">
                SootOption <xsl:value-of select="translate(name[last()],'-. ','___')"/>_branch = new
                SootOption("<xsl:value-of select="name"/>", "<xsl:value-of select="alias"/>");
                root.addChild(<xsl:value-of select="translate(name[last()],'-. ','___')"/>_branch);
                ...
                <xsl:for-each select="(phase|radio_phase)">
                    ...
                    //<xsl:value-of select="name"/>
                    SootOption <xsl:value-of select="translate(alias[last()],'-. ','___')"/>_branch = new
                    SootOption("<xsl:value-of select="name"/>", "<xsl:value-of
                        select="translate(alias[last()],'-. ','___')"/>");
                    ...
                    <xsl:for-each select="sub_phase">
                        SootOption <xsl:value-of select="$parent"/>_<xsl:value-of
                            select="translate(alias[last()],'-. ','___')"/>_branch = new SootOption("<xsl:value-of
                            select="name"/>", "<xsl:value-of select="$phase_alias"/><xsl:value-of
                            select="translate(alias[last()],'-. ','___')"/>");
                        subParent.addChild(<xsl:value-of select="$parent"/>_<xsl:value-of
                            select="translate(alias[last()],'-. ','___')"/>_branch);


                        <!--create branches for sub-phase sub-section pages-->

                        subSectParent = <xsl:value-of select="$parent"/>_<xsl:value-of
                            select="translate(alias[last()],'-. ','___')"/>_branch;

                        <xsl:for-each select="section">
                            SootOption <xsl:value-of select="$parent"/>_<xsl:value-of
                                select="translate((alias|name)[last()],'-. ','___')"/>_branch = new
                            SootOption("<xsl:value-of select="name"/>", "<xsl:value-of
                                select="$phase_alias"/><xsl:value-of select="translate(name[last()],'-. ','___')"/>");

                            subSectParent.addChild(<xsl:value-of select="$parent"/>_<xsl:value-of
                                select="translate((alias|name)[last()],'-. ','___')"/>_branch);
                        </xsl:for-each>
                    ...
                    <xsl:for-each select="sub_phase">
                        ...
                        <xsl:for-each select="section">
                            ...
                            <xsl:call-template name="compInit">
                                ...
                                <xsl:with-param name="parent" select="$phaseAlias"/>
                                <xsl:with-param name="subParent"
                                                select="translate($subPhaseAlias[last()],'-. ','___')"/>
                                <xsl:with-param name="parentAlias" select="$phaseOptAlias"/>
    ...
    <xsl:template mode="pageCon" match="section|phase|radio_phase|sub_phase">
        ...
        <xsl:copy-of select="$java_name"/>Child =
        <xsl:copy-of select="$parent"/>
        <xsl:copy-of select="$java_name"/>Create(getPageContainer());
    </xsl:template>
    ...
    <xsl:template name="createConfig">
        ...
        <xsl:for-each select="boolopt|macroopt">
            boolRes = get<xsl:value-of select="$parent"/><xsl:value-of
                select="translate($subParent[last()],'-. ','___')"/><xsl:value-of
                select="translate(alias[last()],'-. ','___')"/>_widget().getButton().getSelection();
            ...
            getConfig().put(get<xsl:value-of select="$parent"/><xsl:value-of
                select="translate($subParent[last()],'-. ','___')"/><xsl:value-of
                select="translate(alias[last()],'-. ','___')"/>_widget().getAlias(), new Boolean(boolRes));
            }
        </xsl:for-each>


        <xsl:for-each select="stropt|intopt|flopt|listopt">
            stringRes = get<xsl:value-of select="$parent"/><xsl:value-of
                select="translate($subParent[last()],'-. ','___')"/><xsl:value-of
                select="translate(alias[last()],'-. ','___')"/>_widget().getText().getText();
            <xsl:if test="default">
            ...
            if ( (&#33;(stringRes.equals(defStringRes))) &#38;&#38; (stringRes &#33;&#61; null) &#38;&#38;
            (stringRes.length() &#33;&#61; 0)) {
            getConfig().put(get<xsl:value-of select="$parent"/><xsl:value-of
                select="translate($subParent[last()],'-. ','___')"/><xsl:value-of
                select="translate(alias[last()],'-. ','___')"/>_widget().getAlias(), stringRes);
            }
        </xsl:for-each>
        ...
        <xsl:for-each select="multiopt">
            stringRes = get<xsl:value-of select="$parent"/><xsl:value-of
                select="translate($subParent[last()],'-. ','___')"/><xsl:value-of
                select="translate(alias[last()],'-. ','___')"/>_widget().getSelectedAlias();
    ...
    <xsl:template name="compInit">
        ...
        private Composite
        <xsl:value-of select="$parent"/><xsl:value-of select="$callName"/>Create(Composite parent) {
        String defKey;
        ...
        editGroup<xsl:value-of select="$parent"/><xsl:value-of select="$callName"/>.setText("<xsl:value-of
            select="$name"/>");

        editGroup<xsl:value-of select="$parent"/><xsl:value-of select="$callName"/>.setData("id", "<xsl:value-of
            select="$parent"/><xsl:value-of select="$callName"/>");

        String desc<xsl:value-of select="$parent"/><xsl:value-of select="$callName"/> = "
        ...
        Label descLabel<xsl:value-of select="$parent"/><xsl:value-of select="$callName"/> = new Label(editGroup<xsl:value-of
            select="$parent"/><xsl:value-of select="$callName"/>, SWT.WRAP);
        descLabel<xsl:value-of select="$parent"/><xsl:value-of select="$callName"/>.setText(desc<xsl:value-of
            select="$parent"/><xsl:value-of select="$callName"/>);
        }
        OptionData [] data;
        ...
        <xsl:for-each select="boolopt|macroopt">
            ...
            defKey = "<xsl:value-of select="$parentAlias"/>"+" "+"<xsl:value-of select="$subParentAlias"/>"+"
            ...
            set<xsl:value-of select="$parent"/><xsl:value-of select="$subParent"/><xsl:value-of
                select="translate(alias[last()],'-. ','___')"/>_widget(new BooleanOptionWidget(editGroup<xsl:value-of
                select="$parent"/><xsl:value-of select="$callName"/>, SWT.NONE, new OptionData("<xsl:value-of
                select="name"/>", "<xsl:value-of select="$parentAlias"/>", "<xsl:value-of
                select="$subParentAlias"/>","<xsl:value-of select="alias"/>", "
            <xsl:call-template name="format-tooltips">
        ...
        <xsl:for-each select="multiopt">
            ...
            set<xsl:value-of select="$parent"/><xsl:value-of select="$subParent"/><xsl:value-of
                select="translate(alias[last()],'-. ','___')"/>_widget(new MultiOptionWidget(editGroup<xsl:value-of
                select="$parent"/><xsl:value-of select="$callName"/>, SWT.NONE, data, new OptionData("<xsl:value-of
                select="name"/>", "<xsl:value-of select="$parentAlias"/>", "<xsl:value-of
                select="$subParentAlias"/>","<xsl:value-of select="alias"/>", "
            <xsl:call-template name="format-tooltips">
                <xsl:with-param name="text">
            ...
            get<xsl:value-of select="$parent"/><xsl:value-of select="$subParent"/><xsl:value-of
                select="translate(alias[last()],'-. ','___')"/>_widget().setDef(defaultString);
            }
        ...
        <xsl:for-each select="listopt">
            ...
            set<xsl:value-of select="$parent"/><xsl:value-of select="$subParent"/><xsl:value-of
                select="translate(alias[last()],'-. ','___')"/>_widget(new ListOptionWidget(editGroup<xsl:value-of
                select="$parent"/><xsl:value-of select="$callName"/>, SWT.NONE, new OptionData("<xsl:value-of
                select="name"/>", "<xsl:value-of select="$parentAlias"/>", "<xsl:value-of
                select="$subParentAlias"/>","<xsl:value-of select="alias"/>", "
            <xsl:call-template name="format-tooltips">
        ...
        <xsl:for-each select="stropt|intopt|flopt">
            ...
            set<xsl:value-of select="$parent"/><xsl:value-of select="$subParent"/><xsl:value-of
                select="translate(alias[last()],'-. ','___')"/>_widget(new StringOptionWidget(editGroup<xsl:value-of
                select="$parent"/><xsl:value-of select="$callName"/>, SWT.NONE, new OptionData("<xsl:value-of
                select="name"/>", "<xsl:value-of select="$parentAlias"/>", "<xsl:value-of
                select="$subParentAlias"/>","<xsl:value-of select="alias"/>", "
            <xsl:call-template name="format-tooltips">
    ...
    <xsl:template name="objCreation">
        ...
        <xsl:for-each select="boolopt|macroopt">
            private BooleanOptionWidget
            <xsl:value-of select="$parent"/><xsl:value-of
                select="translate($subParent[last()],'-. ','___')"/><xsl:value-of
                select="translate(alias[last()],'-. ','___')"/>_widget;

            private void set<xsl:value-of select="$parent"/><xsl:value-of
                select="translate($subParent[last()],'-. ','___')"/><xsl:value-of
                select="translate(alias[last()],'-. ','___')"/>_widget(BooleanOptionWidget widget) {
            <xsl:value-of select="$parent"/><xsl:value-of
                select="translate($subParent[last()],'-. ','___')"/><xsl:value-of
                select="translate(alias[last()],'-. ','___')"/>_widget = widget;
            }

            public BooleanOptionWidget get<xsl:value-of select="$parent"/><xsl:value-of
                select="translate($subParent[last()],'-. ','___')"/><xsl:value-of
                select="translate(alias[last()],'-. ','___')"/>_widget() {
            return
            <xsl:value-of select="$parent"/><xsl:value-of
        ...
        <xsl:for-each select="listopt">
            ...
            private void set<xsl:value-of select="$parent"/><xsl:value-of
                select="translate($subParent[last()],'-. ','___')"/><xsl:value-of
                select="translate(alias[last()],'-. ','___')"/>_widget(ListOptionWidget widget) {
            <xsl:value-of select="$parent"/><xsl:value-of
                select="translate($subParent[last()],'-. ','___')"/><xsl:value-of
                select="translate(alias[last()],'-. ','___')"/>_widget = widget;
            }

            public ListOptionWidget get<xsl:value-of select="$parent"/><xsl:value-of
                select="translate($subParent[last()],'-. ','___')"/><xsl:value-of
                select="translate(alias[last()],'-. ','___')"/>_widget() {
            return
            <xsl:value-of select="$parent"/><xsl:value-of
        ...
        <xsl:for-each select="stropt|intopt|flopt">
            ...
            private void set<xsl:value-of select="$parent"/><xsl:value-of
                select="translate($subParent[last()],'-. ','___')"/><xsl:value-of
                select="translate(alias[last()],'-. ','___')"/>_widget(StringOptionWidget widget) {
            <xsl:value-of select="$parent"/><xsl:value-of
                select="translate($subParent[last()],'-. ','___')"/><xsl:value-of
                select="translate(alias[last()],'-. ','___')"/>_widget = widget;
            }

            public StringOptionWidget get<xsl:value-of select="$parent"/><xsl:value-of
                select="translate($subParent[last()],'-. ','___')"/><xsl:value-of
                select="translate(alias[last()],'-. ','___')"/>_widget() {
            return
            <xsl:value-of select="$parent"/><xsl:value-of
        ...
        <xsl:for-each select="multiopt">
            ...
            private void set<xsl:value-of select="$parent"/><xsl:value-of
                select="translate($subParent[last()],'-. ','___')"/><xsl:value-of
                select="translate(alias[last()],'-. ','___')"/>_widget(MultiOptionWidget widget) {
            <xsl:value-of select="$parent"/><xsl:value-of
                select="translate($subParent[last()],'-. ','___')"/><xsl:value-of
                select="translate(alias[last()],'-. ','___')"/>_widget = widget;
            }

            public MultiOptionWidget get<xsl:value-of select="$parent"/><xsl:value-of
                select="translate($subParent[last()],'-. ','___')"/><xsl:value-of
                select="translate(alias[last()],'-. ','___')"/>_widget() {
            return
            <xsl:value-of select="$parent"/><xsl:value-of
