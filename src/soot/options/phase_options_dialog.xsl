...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
    ...
    <xsl:template match="/options">
        ...
        <xsl:for-each select="section/phaseopt/radio_phase">
            ...
            <xsl:for-each select="sub_phase">
                ...
                <xsl:for-each select="boolopt">
                    ...
                    buttonList.add(get<xsl:value-of select="$phase_alias"/><xsl:value-of select="$sub_phase_alias"/><xsl:value-of
        ...
        <xsl:for-each select="section/phaseopt">
            <xsl:for-each select="(phase|radio_phase)">
                ...
                <xsl:for-each select="(boolopt|stringopt|intopt|flopt|multiopt)">
                    addToEnableGroup("<xsl:value-of select="$phase_alias"/>", get<xsl:value-of select="$phase_alias"/><xsl:value-of
                ...
                <xsl:for-each select="sub_phase">
                    ...
                    <xsl:for-each select="(boolopt|stringopt|intopt|flopt|multiopt)">
                        addToEnableGroup("<xsl:value-of select="$phase_alias"/>", "<xsl:value-of select="$sub_phase_alias"/>", get<xsl:value-of
                            select="$phase_alias"/><xsl:value-of select="$sub_phase_alias_translated"/><xsl:value-of
                            select="translate(alias[last()],'-. ','___')"/>_widget(), "<xsl:value-of select="alias"/>");
                    </xsl:for-each>

                    <xsl:for-each select="boolopt">
                        get<xsl:value-of select="$phase_alias"/><xsl:value-of select="$sub_phase_alias_translated"/><xsl:value-of
                            select="translate(alias[last()],'-. ','___')"/>_widget().getButton().addSelectionListener(this);
                    </xsl:for-each>
        ...
        */
        ...
        <xsl:for-each select="section">
            ...
            "<xsl:value-of select="translate(name[last()],'-. ','___')"/>");
            ...
            <xsl:for-each select="phaseopt">
                SootOption <xsl:value-of select="translate(name[last()],'-. ','___')"/>_branch = new SootOption("<xsl:value-of
                    select="name"/>", "<xsl:value-of select="alias"/>");
                root.addChild(<xsl:value-of select="translate(name[last()],'-. ','___')"/>_branch);
                ...
                <xsl:for-each select="(phase|radio_phase)">
                    ...
                    //<xsl:value-of select="name"/>
                    SootOption <xsl:value-of select="translate(alias[last()],'-. ','___')"/>_branch = new SootOption("<xsl:value-of
                        select="name"/>", "<xsl:value-of select="translate(alias[last()],'-. ','___')"/>");
                    ...
                    <xsl:for-each select="sub_phase">
                        SootOption <xsl:value-of select="$parent"/>_<xsl:value-of select="translate(alias[last()],'-. ','___')"/>_branch =
                        new SootOption("<xsl:value-of select="name"/>", "<xsl:value-of select="$phase_alias"/><xsl:value-of
                        ...
                        <xsl:for-each select="section">
                            SootOption <xsl:value-of select="$parent"/>_<xsl:value-of select="translate((alias|name)[last()],'-. ','___')"/>_branch
                            = new SootOption("<xsl:value-of select="name"/>", "<xsl:value-of select="$phase_alias"/><xsl:value-of
                                select="translate(name[last()],'-. ','___')"/>");
    ...
    <xsl:template name="createConfig">
        ...
        <xsl:for-each select="boolopt|macroopt">
            boolRes = get<xsl:value-of select="$parent"/><xsl:value-of select="translate($subParent[last()],'-. ','___')"/><xsl:value-of
            ...
            getConfig().put(get<xsl:value-of select="$parent"/><xsl:value-of
                select="translate($subParent[last()],'-. ','___')"/><xsl:value-of select="translate(alias[last()],'-. ','___')"/>_widget().getAlias(),
            new Boolean(boolRes));
            }
        </xsl:for-each>
        ...
        <xsl:for-each select="stropt|intopt|flopt|listopt">
            stringRes = get<xsl:value-of select="$parent"/><xsl:value-of select="translate($subParent[last()],'-. ','___')"/><xsl:value-of
            ...
            if ( (&#33;(stringRes.equals(defStringRes))) &#38;&#38; (stringRes &#33;&#61; null) &#38;&#38; (stringRes.length() &#33;&#61;
            0)) {
            getConfig().put(get<xsl:value-of select="$parent"/><xsl:value-of
                select="translate($subParent[last()],'-. ','___')"/><xsl:value-of select="translate(alias[last()],'-. ','___')"/>_widget().getAlias(),
            stringRes);
    ...
    <xsl:template name="compInit">
        ...
        <xsl:for-each select="boolopt|macroopt">
            ...
            defKey = "<xsl:value-of select="$parentAlias"/>"+" "+"<xsl:value-of select="$subParentAlias"/>"+" "+"<xsl:value-of
                select="alias"/>";
            defKey = defKey.trim();
            ...
            set<xsl:value-of select="$parent"/><xsl:value-of select="$subParent"/><xsl:value-of
                ...
                select="$parent"/><xsl:value-of select="$callName"/>, SWT.NONE, new OptionData("<xsl:value-of select="name"/>",
            "<xsl:value-of select="$parentAlias"/>", "<xsl:value-of select="$subParentAlias"/>","<xsl:value-of select="alias"/>", "
        ...
        <xsl:for-each select="multiopt">
            ...
            set<xsl:value-of select="$parent"/><xsl:value-of select="$subParent"/><xsl:value-of
                ...
                select="$parent"/><xsl:value-of select="$callName"/>, SWT.NONE, data, new OptionData("<xsl:value-of select="name"/>",
            "<xsl:value-of select="$parentAlias"/>", "<xsl:value-of select="$subParentAlias"/>","<xsl:value-of select="alias"/>", "
            <xsl:call-template name="format-tooltips">
                <xsl:with-param name="text">
                    <xsl:apply-templates select="long_desc"/>
    ...
    <xsl:template name="objCreation">
        ...
        <xsl:for-each select="boolopt|macroopt">
            ...
            <xsl:value-of select="$parent"/><xsl:value-of select="translate($subParent[last()],'-. ','___')"/><xsl:value-of
                select="translate(alias[last()],'-. ','___')"/>_widget;

            private void set<xsl:value-of select="$parent"/><xsl:value-of select="translate($subParent[last()],'-. ','___')"/><xsl:value-of
                select="translate(alias[last()],'-. ','___')"/>_widget(BooleanOptionWidget widget) {
            <xsl:value-of select="$parent"/><xsl:value-of select="translate($subParent[last()],'-. ','___')"/><xsl:value-of
                select="translate(alias[last()],'-. ','___')"/>_widget = widget;
            }

            public BooleanOptionWidget get<xsl:value-of select="$parent"/><xsl:value-of select="translate($subParent[last()],'-. ','___')"/><xsl:value-of
                select="translate(alias[last()],'-. ','___')"/>_widget() {
            return
            <xsl:value-of select="$parent"/><xsl:value-of select="translate($subParent[last()],'-. ','___')"/><xsl:value-of
                select="translate(alias[last()],'-. ','___')"/>_widget;
        ...
        <xsl:for-each select="listopt">
            ...
            public ListOptionWidget get<xsl:value-of select="$parent"/><xsl:value-of
                select="translate($subParent[last()],'-. ','___')"/><xsl:value-of select="translate(alias[last()],'-. ','___')"/>_widget() {
            return
            <xsl:value-of select="$parent"/><xsl:value-of select="translate($subParent[last()],'-. ','___')"/><xsl:value-of
                select="translate(alias[last()],'-. ','___')"/>_widget;
            }
